...
const CSRF = window.INIT_DATA.csrf;
const CUR_USER = window.INIT_DATA.curUser;
const INIT_TICKETS = window.INIT_DATA.tickets;
const INIT_CLIENTS = window.INIT_DATA.clients;
const INIT_IT_TEAM = window.INIT_DATA.itTeam;
const INIT_CONFIG  = window.INIT_DATA.config;
const INIT_AA      = window.INIT_DATA.aa;
...
function renderSLAPanel(){
  ...
  el.innerHTML=sorted.slice(0,6).map(t=>{
    const sla=t.sla||{};
    const pct=sla.pct||0;
    const statusCls=sla.cls==='sla-over'?'sla-over':sla.cls==='sla-warn'?'sla-warn':'sla-ok';
    const warnCls=sla.cls==='sla-over'?'crit':sla.cls==='sla-warn'?'warn':'ok';
    const warnIcon=sla.cls==='sla-over'?'⚠️':sla.cls==='sla-warn'?'⏰':'✓';
    const barGrad=sla.cls==='sla-over'?'linear-gradient(90deg,#EF4444,#F87171)':sla.cls==='sla-warn'?'linear-gradient(90deg,#F59E0B,#FBBF24)':'linear-gradient(90deg,#10B981,#34D399)';
    return `<div class="sla-row-item ${statusCls}" style="cursor:pointer" onclick="openDetail('${t.id}')">
      ...
      <div class="sla-mini-bar"><div class="sla-mini-fill" style="width:${pct}%;background:${barGrad}"></div></div>
...
function renderUserDashboard(){
  ...
  if(sq) filtered=filtered.filter(t=>t.id.toLowerCase().includes(sq)||t.title.toLowerCase().includes(sq)||(t.assignee||'').toLowerCase().includes(sq));
  ...
  if(slaList){
    if(!active.length){
      ...
      slaList.innerHTML=active.sort((a,b)=>(b.sla?.pct||0)-(a.sla?.pct||0)).slice(0,8).map(t=>{
        ...
        const warnIcon=sla.cls==='sla-over'?'⚠️':sla.cls==='sla-warn'?'⏰':'✓';
        const barGrad=sla.cls==='sla-over'?'linear-gradient(90deg,#EF4444,#F87171)':sla.cls==='sla-warn'?'linear-gradient(90deg,#F59E0B,#FBBF24)':'linear-gradient(90deg,#10B981,#34D399)';
        return `<div class="sla-row-item ${statusCls}" onclick="openDetail('${t.id}')">
          <div class="sla-row-top">
            <span class="sla-row-title" title="${t.title}">${t.id} · ${t.title}</span>
            <span class="sla-row-label ${sla.cls||''}">${sla.label||'—'}</span>
          </div>
          <div class="sla-mini-bar"><div class="sla-mini-fill" style="width:${sla.pct||0}%;background:${barGrad}"></div></div>
          <div class="ud-sla-meta">
...
function renderTable(){
  ...
  const typeLbl=type=>type==='incident'?'Insiden':type==='newproject'?'Proyek Baru':'Permintaan';
  tbody.innerHTML=page.map(t=>`
    <tr onclick="openDetail('${t.id}')">
      <td><span class="tbl-id">${t.id}</span></td>
      <td class="tbl-title-cell">
        <div class="tbl-name">${t.title}</div>
        <span class="tag ${t.type}" style="font-size:9.5px;padding:1px 5px">${typeLbl(t.type)}</span>
      </td>
      ...
      <td><button class="tbl-detail-btn" onclick="event.stopPropagation();openDetail('${t.id}')">Detail →</button></td>
    </tr>`).join('');
...
function submitTicket(){
  ...
  setLoading(btn,true);
  api(window.ROUTES.ticketsStore,{method:'POST',body:form}).then(r=>{
...
function reloadTickets(){
  fetch(window.ROUTES.appData,{headers:{'Accept':'application/json','X-CSRF-TOKEN':CSRF}})
...
function saveAutoAssign(){
  ...
  apiJson(window.ROUTES.autoassignStore,'POST',data).then(r=>{if(r.success){reloadTickets();openAutoAssign();showToast('✅ Aturan berhasil disimpan');}});
}
...
function addClient(){
  ...
  if(!nama) return;
  apiJson(window.ROUTES.clientsStore,'POST',{nama}).then(r=>{if(r.success){document.getElementById('new-client-name').value='';reloadTickets();openAutoAssign();showToast('✅ Klien berhasil ditambahkan');}});
}
...
function openUserManagement(){
  fetch(window.ROUTES.usersData,{headers:{'Accept':'application/json','X-CSRF-TOKEN':CSRF}})
...
function addUser(){
  ...
  setLoading(btn,true);
  apiJson(window.ROUTES.usersStore,'POST',data).then(r=>{
    setLoading(btn,false);
    if(r.success){
      // reset form
...
function saveAppSettings(){
  ...
  apiJson(window.ROUTES.configSave,'POST',APP_CONFIG).then(r=>{
...
function resetAppSettings(){
  ...
  apiJson(window.ROUTES.configReset,'POST').then(r=>{
...
function savePassword(){
  ...
  setLoading(btn,true);
  apiJson(window.ROUTES.passwordChange,'POST',{old_password:old,new_password:nw,new_password_confirmation:cf}).then(r=>{
    setLoading(btn,false);
...
function initEcho() {
  ...
  try {
    _echo = new Echo({
      ...
      wsPort:      rd.port,
      wssPort:     rd.port,
      forceTLS:    rd.scheme === 'https',
      enabledTransports: rd.scheme === 'https' ? ['wss'] : ['ws'],
      authEndpoint: '/broadcasting/auth',
      auth: { headers: { 'X-CSRF-TOKEN': CSRF } },
    });

    _echo.private(`notifications.${curUser.id}`)
      .listen('.notification.sent', (data) => {
        // Notifikasi real-time masuk — update badge dan list
        _notifCount = Math.max(0, _notifCount) + 1;
        updateNotifBadge(_notifCount);
        showToast(`🔔 ${data.title}`);
        pollNotifications(); // refresh full list dari server
      });
  } catch (e) {
    // Reverb tidak tersedia, fallback ke polling saja
  }
}
...
// Inisialisasi Echo (WebSocket) — polling hanya sebagai fallback setiap 5 menit
initEcho();
setTimeout(() => { pollNotifications(); setInterval(pollNotifications, 300000); }, 1000);
...
const _chartFontFamily = 'Space Grotesk';
const _chartMonoFamily = 'JetBrains Mono';

const _chartBaseOpts = (opts = {}) => Object.assign({
  responsive: true,
  maintainAspectRatio: false,
  plugins: { legend: { position: 'bottom', labels: { font: { family: _chartFontFamily, size: 11 }, padding: 10, boxWidth: 10 } } }
}, opts);

const _chartScales = () => ({
  y: { beginAtZero: true, ticks: { stepSize: 1, font: { family: _chartMonoFamily, size: 10 } }, grid: { color: 'rgba(0,0,0,0.05)' } },
  x: { ticks: { font: { family: _chartFontFamily, size: 11 } }, grid: { display: false } }
});

function initCharts() {
  const t = curUser.type;
  if (t === 'user') _initUserCharts();
  else if (t === 'it') _initITCharts();
  else if (t === 'manager') _initManagerCharts();
  else if (t === 'it_manager') _initITManagerCharts();
}

function _initUserCharts() {
  const mine = tickets.filter(t => t.creator_id === curUser.id);
  const nPend   = mine.filter(t => t.approval === 'pending').length;
  const nActive = mine.filter(t => t.approval === 'approved' && !t.closed_at).length;
  const nClosed = mine.filter(t => !!t.closed_at).length;

  _mkChart('chart-user-donut', {
    type: 'doughnut',
    data: {
      labels: ['Menunggu', 'Berjalan', 'Selesai'],
      datasets: [{ data: [nPend, nActive, nClosed], backgroundColor: ['#F59E0B','#6366F1','#10B981'], borderWidth: 2, borderColor: '#fff', hoverOffset: 6 }]
    },
    options: Object.assign(_chartBaseOpts(), { cutout: '70%', plugins: { legend: { position: 'bottom', labels: { font: { family: _chartFontFamily, size: 11 }, padding: 10, boxWidth: 10 } }, tooltip: { callbacks: { label: c => ` ${c.label}: ${c.raw} tiket` } } } })
  });
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    counts.push(mine.filter(t => { const cd = new Date(t.created_at); return cd.getFullYear() === d.getFullYear() && cd.getMonth() === d.getMonth(); }).length);
  }

  _mkChart('chart-user-bar', {
    type: 'bar',
    data: { labels: months, datasets: [{ label: 'Tiket Dibuat', data: counts, backgroundColor: 'rgba(99,102,241,0.75)', borderRadius: 6, borderSkipped: false }] },
    options: Object.assign(_chartBaseOpts({ plugins: { legend: { display: false } } }), { scales: _chartScales() })
  });
}

function _initITCharts() {
  const leftTitleEl = document.getElementById('it-chart-left-title');
  const rightTitleEl = document.getElementById('it-chart-right-title');
  if (leftTitleEl) leftTitleEl.textContent = 'Kategori Tiket';
  if (rightTitleEl) rightTitleEl.textContent = 'Dibuat per Hari (7 hari)';

  const catMap = {};
  tickets.forEach(t => { const k = t.category || 'Lainnya'; catMap[k] = (catMap[k] || 0) + 1; });
  const catColors = ['#6366F1','#EC4899','#F97316','#10B981','#F59E0B','#7C3AED','#0891B2','#EF4444'];
  const catLabels = Object.keys(catMap);

  _mkChart('chart-it-left', {
    type: 'doughnut',
    data: { labels: catLabels, datasets: [{ data: Object.values(catMap), backgroundColor: catColors.slice(0, catLabels.length), borderWidth: 2, borderColor: '#fff', hoverOffset: 6 }] },
    options: Object.assign(_chartBaseOpts(), { cutout: '65%' })
  });
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    dayCounts.push(tickets.filter(t => { const cd = new Date(t.created_at); cd.setHours(0,0,0,0); return cd.getTime() === d.getTime(); }).length);
  }

  _mkChart('chart-it-right', {
    type: 'bar',
    data: { labels: days, datasets: [{ label: 'Tiket', data: dayCounts, backgroundColor: 'rgba(236,72,153,0.7)', borderRadius: 6, borderSkipped: false }] },
    options: Object.assign(_chartBaseOpts({ plugins: { legend: { display: false } } }), { scales: _chartScales() })
  });
}

function _initManagerCharts() {
  const leftTitleEl = document.getElementById('it-chart-left-title');
  const rightTitleEl = document.getElementById('it-chart-right-title');
  if (leftTitleEl) leftTitleEl.textContent = 'Status Tim';
  if (rightTitleEl) rightTitleEl.textContent = 'Tren Tiket per Minggu';

  const nPend   = tickets.filter(t => t.approval === 'pending').length;
  const nActive = tickets.filter(t => t.approval === 'approved' && !t.closed_at).length;
  const nClosed = tickets.filter(t => !!t.closed_at).length;

  _mkChart('chart-it-left', {
    type: 'doughnut',
    data: { labels: ['Menunggu', 'Berjalan', 'Selesai'], datasets: [{ data: [nPend, nActive, nClosed], backgroundColor: ['#F59E0B','#6366F1','#10B981'], borderWidth: 2, borderColor: '#fff', hoverOffset: 6 }] },
    options: Object.assign(_chartBaseOpts(), { cutout: '70%' })
  });

  const now = new Date(); now.setHours(0,0,0,0);
  const weeks = [], created = [], completed = [];
  for (let i = 7; i >= 0; i--) {
    const wEnd = new Date(now); wEnd.setDate(wEnd.getDate() - i * 7);
    const wStart = new Date(wEnd); wStart.setDate(wStart.getDate() - 7);
    weeks.push('W' + (8 - i));
    created.push(tickets.filter(t => { const d = new Date(t.created_at); return d >= wStart && d < wEnd; }).length);
    completed.push(tickets.filter(t => { if (!t.closed_at) return false; const d = new Date(t.closed_at); return d >= wStart && d < wEnd; }).length);
  }
  ...
  _mkChart('chart-it-right', {
    ...
    data: {
      labels: weeks,
      datasets: [
        { label: 'Dibuat',      data: created,    borderColor: '#6366F1', backgroundColor: 'rgba(99,102,241,0.08)', tension: 0.4, pointRadius: 4, pointBackgroundColor: '#6366F1', fill: true },
        { label: 'Diselesaikan',data: completed,  borderColor: '#10B981', backgroundColor: 'rgba(16,185,129,0.08)', tension: 0.4, pointRadius: 4, pointBackgroundColor: '#10B981', fill: true },
      ]
    },
    options: Object.assign(_chartBaseOpts(), { scales: _chartScales() })
  });
}

function _initITManagerCharts() {
  const leftTitleEl = document.getElementById('it-chart-left-title');
  const rightTitleEl = document.getElementById('it-chart-right-title');
  if (leftTitleEl) leftTitleEl.textContent = 'Workload per Anggota';
  if (rightTitleEl) rightTitleEl.textContent = 'Distribusi Tipe Tiket';

  const memberMap = {};
  IT_TEAM.forEach(m => { memberMap[m.name] = { active: 0, pending: 0 }; });
  tickets.forEach(t => {
    if (!t.assignee || !memberMap[t.assignee]) return;
    if (t.approval === 'pending') memberMap[t.assignee].pending++;
    else if (!t.closed_at) memberMap[t.assignee].active++;
  });
  const members = Object.keys(memberMap).slice(0, 8);

  _mkChart('chart-it-left', {
    type: 'bar',
    data: {
      labels: members.map(n => n.split(' ')[0]),
      datasets: [
        { label: 'Aktif',     data: members.map(m => memberMap[m].active),  backgroundColor: 'rgba(99,102,241,0.8)',  borderRadius: 4, borderSkipped: false },
        { label: 'Menunggu',  data: members.map(m => memberMap[m].pending), backgroundColor: 'rgba(245,158,11,0.7)', borderRadius: 4, borderSkipped: false },
      ]
    },
    options: Object.assign(_chartBaseOpts(), { scales: _chartScales() })
  });

  const nInc  = tickets.filter(t => t.type === 'incident').length;
  const nNew  = tickets.filter(t => t.type === 'newproject').length;
  const nReq  = tickets.filter(t => t.type === 'openrequest').length;

  _mkChart('chart-it-right', {
    type: 'pie',
    data: { labels: ['Insiden', 'Proyek Baru', 'Permintaan'], datasets: [{ data: [nInc, nNew, nReq], backgroundColor: ['#EF4444','#6366F1','#F97316'], borderWidth: 2, borderColor: '#fff', hoverOffset: 6 }] },
...
// ── Expose fungsi ke global scope untuk inline HTML handlers ──
window.addClient = addClient;
window.addUser = addUser;
window.addTask = addTask;
window.approveFreezeReq = approveFreezeReq;
window.approveTix = approveTix;
window.closeM = closeM;
window.closeTix = closeTix;
window.deleteAA = deleteAA;
window.deleteClient = deleteClient;
window.deleteTix = deleteTix;
window.deleteTask = deleteTask;
window.deleteUser = deleteUser;
window.dismissRejectionNotice = dismissRejectionNotice;
window.doApproveTix = doApproveTix;
window.doRejectTix = doRejectTix;
window.markAllNotifRead = markAllNotifRead;
window.onNotifClick = onNotifClick;
window.openApprovalQueue = openApprovalQueue;
window.openAppSettings = openAppSettings;
window.openAutoAssign = openAutoAssign;
...
window.rejectFreezeReq = rejectFreezeReq;
window.rejectTix = rejectTix;
window.removeFile = removeFile;
window.resetAppSettings = resetAppSettings;
window.saveAppSettings = saveAppSettings;
window.saveAutoAssign = saveAutoAssign;
window.saveEditTask = saveEditTask;
window.savePassword = savePassword;
window.selectEmoji = selectEmoji;
window.sendComment = sendComment;
window.setBgColor = setBgColor;
window.setBgGradient = setBgGradient;
window.setListPage = setListPage;
window.setListPerPage = setListPerPage;
window.setTF = setTF;
window.submitFreeze = submitFreeze;
window.submitTicket = submitTicket;
window.switchAATab = switchAATab;
...
window.udSetTab = udSetTab;
window.udSetType = udSetType;
window.unfreezeTix = unfreezeTix;
window.searchTix = searchTix;
