...
:root{
  ...
  --border:#cce9f3;--red:#dc2626;--red-bg:rgba(220,38,38,0.08);--radius:12px;
}
...
input[type=password],input[type=text]{width:100%;background:#fff;border:1.5px solid var(--border);border-radius:10px;padding:12px 42px 12px 16px;color:var(--text);font-size:14px;font-family:inherit;outline:none;transition:border-color .2s,box-shadow .2s;}
input:focus{border-color:var(--blue);box-shadow:0 0 0 3px rgba(27,158,201,0.12);}
input.err{border-color:var(--red);box-shadow:0 0 0 3px rgba(220,38,38,0.08);}
.eye-btn{position:absolute;right:13px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--text3);cursor:pointer;font-size:16px;padding:2px;}
...
.error-box{background:var(--red-bg);border:1px solid rgba(220,38,38,0.25);border-radius:9px;padding:10px 14px;font-size:12px;color:var(--red);margin-bottom:18px;}
</style>
</head>
<body>
<div class="box">
  <div class="card">
    ...
    <p class="sub">Masukkan password baru Anda. Minimal 8 karakter.</p>
    ...
    <form method="POST" action="{{ route('password.reset.post', $token) }}" onsubmit="this.querySelector('.btn').disabled=true">
      @csrf
      <div class="form-group">
        ...
        <div class="input-wrap">
          ...
          <button type="button" class="eye-btn" onclick="togglePw('pw1',this)">👁</button>
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="input-wrap">
          ...
          <button type="button" class="eye-btn" onclick="togglePw('pw2',this)">👁</button>
        </div>
      </div>
